...
// const API_BASE_URL= import.meta.env.VITE_API_BASE_URL || 'https://karvaantours.com/api';
...
// Tours API calls (UPDATED ROUTES from /posts to /tours)
export const postsAPI = {
  getPosts: async (params?: any) => {
    try {
  ...
  createPost: async (postData: any) => {
    try {
      ...
      const response = await api.post('/tours', enhancedPostData);
      return response.data;
    } catch (error: any) {
      if (error.response?.data?.errors && Array.isArray(error.response.data.errors)) {
        error.response.data.errors.forEach((err: string, index: number) => {
  ...
  updatePost: async (id: string, postData: any) => {
    try {
      const response = await api.put(`/tours/${id}`, postData);
      return response.data;
    } catch (error: any) {
      console.error('❌ Update API Error:', error.response?.data);
      return { 
  ...
  addReview: async (id: string, review: { rating: number; comment: string }) => {
    const response = await api.post(`/tours/${id}/reviews`, review);
    return response.data;
  },
  ...
  getRatings: async (tourId: string) => {
    try {
      const response = await api.get(`/tours/${tourId}/ratings`);
      return response.data;
    } catch (error: any) {
      console.error('❌ API Error:', error.response?.data || error.message);
      return {
  ...
  addRating: async (tourId: string, ratingData: { 
    ...
    try {
      const response = await api.post(`/tours/${tourId}/ratings`, ratingData);
      return response.data;
    } catch (error: any) {
      console.error('❌ API Error:', error.response?.data || error.message);
      return {
  ...
  updateRating: async (tourId: string, ratingId: string, updateData: { 
    ...
    try {
      const response = await api.put(`/tours/${tourId}/ratings/${ratingId}`, updateData);
      return response.data;
    } catch (error: any) {
      console.error('❌ API Error:', error.response?.data || error.message);
      return {
  ...
  deleteRating: async (tourId: string, ratingId: string) => {
    try {
      const response = await api.delete(`/tours/${tourId}/ratings/${ratingId}`);
      return response.data;
    } catch (error: any) {
      console.error('❌ API Error:', error.response?.data || error.message);
      return {
...
export const adminAPI = {
  ...
  togglePostFeatured: async (id: string, featured: boolean) => {
    const response = await api.put(`/admin/tours/${id}/feature`, { featured });
    return response.data;
  },
