...
const TourDetailPage = () => {
  ...
  // Images - Memoized to prevent re-calculation (BEFORE early returns)
  ...
  const includesArray = useMemo(() => {
    if (!tour || !tour.includes) return [];
    return Array.isArray(tour.includes) 
      ? tour.includes 
      : tour.includes.split('\n').filter(item => item.trim());
  }, [tour]);

  const excludesArray = useMemo(() => {
    if (!tour || !tour.excludes) return [];
    return Array.isArray(tour.excludes) 
      ? tour.excludes 
      : tour.excludes.split('\n').filter(item => item.trim());
  }, [tour]);
              ...
              <div className="grid grid-cols-3 gap-4">
                {thumbnailImages.map((img, idx) => (
                  <div
                    ...
                    className={`relative aspect-video rounded-xl overflow-hidden cursor-pointer transition-all ${activeImageIndex === idx ? 'ring-2 ring-green-500' : 'opacity-70 hover:opacity-100'
                      }`}
                  >
                    <img
                      src={img}
      ...
      Selected Selling Points
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Location Details */}
            <div className="bg-white p-4 rounded-lg shadow-sm">
              <h3 className="text-lg font-bold text-gray-800 mb-3">Itinery</h3>
              <div className="space-y-3">
                {tour.pickupLocation && (
            ...
            )}{/* Additional Images Gallery - Add this section after Rating Component */}
{tour.additionalImages && tour.additionalImages.length > 0 && (
  <div className="mt-8">
    <h3 className="text-2xl font-bold text-gray-900 mb-6">Tour Gallery</h3>
